...
import { TFastApiRequest, TResult } from "@/baselib/aPublic/PublicClass"
    ...
    public static async httpDoAsync(qRequest: THTTPRequest): Promise<TResult> {
        ...
        try {
            var response = await axios.request({
                ...
                headers: qRequest.headers,
                data: qRequest.data,
                responseType: qRequest.resultFormat == TResultFormat.fileDown ? "blob" : undefined
            });

            if (response.status == 200) {
                let lContentType = response.headers['content-type'];
                if (qRequest.resultFormat == TResultFormat.fileDown || lContentType.indexOf('octet-stream') >= 0) {
                    if (lContentType.indexOf('octet-stream') >= 0) {
                        ...
                        let lBlob = new Blob([response.data], { type: 'application/octet-stream;charset=UTF-8' })
                        ...
                        //用异步的方式，让代码更容易写
                        let lTempResult = await new Promise<string>((resolve, reject) => {
                            //转化成标准JSON
                            let lReader = new FileReader()
                            lReader.onload = function (event) {
                                if (lReader.result != null) {
                                    return resolve(lReader.result.toString());
                                }
                            }
                            lReader.readAsText(response.data);
                        })
                        let lTempJosn = JSON.parse(lTempResult);
                        if (qRequest.isResult && lTempJosn.resultCode) {
                            lResult.resultCode = lTempJosn.resultCode;
                            lResult.resultMsg = lTempJosn.resultMsg;
                            lResult.resultFormat = lTempJosn.resultFormat;
                            lResult.resultData = lTempJosn.resultData;
                            lResult.resultDataPageCount = lTempJosn.resultDataPageCount;
                            if (lResult.resultCode == TResultCode.resultTrue) {
                                lResult.resultSuccess = true;
                            }
                        } else {
                            //非标准的Result格式
                            lResult.resultData = lTempJosn;
                        }
                        return lResult;
                    }
                }
